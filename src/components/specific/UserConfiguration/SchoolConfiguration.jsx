...
const SchoolConfiguration = ({ schoolData, setSchoolData, isEditingConfig, setIsEditingConfig,handleSave }) => {
  ...
  const handleDayToggle = (dayValue) => {
    ...
    setSchoolData(prev => {
      ...
      const totalSlots = newDaySchedules.reduce(
        (sum, schedule) => sum + schedule.teaching_slots, 0
      );
  ...
  const handleSlotChange = (day, value) => {
    ...
    setSchoolData(prev => {
      const newDaySchedules = prev.academic_schedule?.day_schedules?.map(schedule =>
        schedule.day === day ? { ...schedule, teaching_slots: slots } : schedule
      );
      ...
      <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', md: '1fr 1fr' }, gap: 3 }}>
        <Paper elevation={0} sx={{ p: 2,  backgroundColor:'transparent'}}>
          ...
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
            {DAYS_OF_WEEK.map((day) => {
              const daySchedule = schoolData?.academic_schedule?.day_schedules?.find(
                ...
                >
                  ...
                  {isChecked && (
                    <TextField
                      ...
                      onChange={(e) => handleSlotChange(day.value, e.target.value)}
                      disabled={!isEditingConfig}
                      InputProps={{
        ...
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
          <TextField
            ...
            value={schoolData?.academic_schedule?.total_weekly_teaching_slots || 0}
            disabled
            size="small"
            fullWidth
            InputLabelProps={{
          ...
          <TextField
            ...
            onChange={(e) => handleInputChange('average_students_allowed', e.target.value)}
            disabled={!isEditingConfig}
            size="small"
            fullWidth
            InputLabelProps={{
