...
function handleSubmitSwapRooms(session1Param, session2Param) {
  ...
  const instructor1 = teacherWeekTimetable?.[selectedDay]?.find(
    (teacher) => teacher.instructor.id === fromRoom.teacherId
  );
  const instructor2 = teacherWeekTimetable?.[selectedDay]?.find(
  ...
  const updatedTeacherWeekTimetable = JSON.parse(
    JSON.stringify(teacherWeekTimetable)
  );

  // Find the specific instructor session arrays in the updated timetable
  const updatedInstructor1 = updatedTeacherWeekTimetable[selectedDay].find(
    (teacher) => teacher.instructor.id === fromRoom.teacherId
  );
  const updatedInstructor2 = updatedTeacherWeekTimetable[selectedDay].find(
    (teacher) => teacher.instructor.id === toRoom.teacherId
  );
  ...
  updatedInstructor1.sessions[fromRoom.sessionGrpIdx][
    ...
    updatedInstructor2.sessions[toRoom.sessionGrpIdx][toRoom.sessionIndex].room;
  updatedInstructor2.sessions[toRoom.sessionGrpIdx][toRoom.sessionIndex].room =
    tempRoom;

  // Update the state with the modified timetable
  setTeacherWeekTimetable(updatedTeacherWeekTimetable);
...
function handleSubmitAddRoom(sessionParam) {
  ...
  const instructor = teacherWeekTimetable?.[selectedDay]?.find(
    (teacher) => teacher.instructor.id === fromRoom.teacherId
  );
  ...
  updatedTeacherWeekTimetable[selectedDay].find(
    (teacher) => teacher.instructor.id === fromRoom.teacherId
...
const RoomBadge = ({ room }) => {
  ...
  return (
    <div className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-blue-900/20 dark:to-indigo-900/20 rounded-lg border border-blue-100 dark:border-blue-800">
      ...
      <span className="text-blue-600 dark:text-blue-400">{room.room_type}</span>
    </div>
  );
};
  const showPreview = roomChangeDialogOpen?.toRoom && roomChangeDialogOpen?.type;
